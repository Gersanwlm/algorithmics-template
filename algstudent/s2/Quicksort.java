...
public class Quicksort {
	...
	public static void quicksortK(int[] a,int k) {
	...
	public static void quicksortK(int[] a, int left, int right, int k) {
		int i = left;
		int j = right - 1;
		int pivot;
		...
		if (left < right){ //if there is one element it is not necessary
			...
			if ((right - left) >= 3){ 
				...
				//we set the position of the pivot
				Vector.interchange(a, i, right);
				QuicksortInsertion.quicksortInsertion(a, left, i-1,k);
				QuicksortInsertion.quicksortInsertion(a, i+1, right,k);		
			} //if
		} //if
	}
